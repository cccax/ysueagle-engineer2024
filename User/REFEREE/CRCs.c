...
/*==============================================================================
  ...
  ==============================================================================
    [..]  该部分提供如下函数:
		  (+) 8位CRC校验码计算运算函数 CRC08_Calculate
      (+) 8位CRC校验码验证验算函数 CRC08_Verify
			(+) 8位CRC校验码末尾添加函数 CRC08_Append
*/
uint8_t CRC08_Calculate(uint8_t *Data, uint32_t Lenth /* Without check code lenth 1 */)
{
	uint8_t CheckCode = CRC08_INIT;
	while (Lenth --) CheckCode = CRC08_Table[CheckCode ^ *Data ++];
	return CheckCode;
}
...
/*==============================================================================
  ...
  ==============================================================================
    [..]  该部分提供如下函数:
		  (+) 16位CRC校验码计算运算函数 CRC16_Calculate
      (+) 16位CRC校验码验证验算函数 CRC16_Verify
			(+) 16位CRC校验码末尾添加函数 CRC16_Append
*/
uint16_t CRC16_Calculate(uint8_t *Data, uint32_t Lenth /* Without check code lenth 2 */)
{
	...
	while(Lenth --) CheckCode = CheckCode >> 8 ^ CRC16_Table[(CheckCode ^ (uint16_t)*Data ++) & 0x00ff];
	return CheckCode;
}
