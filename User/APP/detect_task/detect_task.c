/**
  ...
  * @brief      共有两种设备错误方式：设备离线和数据接收异常。
				设备离线判断任务，通过freeRTOS滴答时间作为系统时间，设备获取数据后
  *             调用DetectHook记录对应设备的时间，在该任务会通过判断记录时间与系统
  *             时间之差来判断掉线。
				数据接收异常判断：通过钩子函数，写一个检测数据异常的逻辑函数，这个函
				数返回一个BOOL变量，然后做相应处理，所有的东西都在一个结构体里，方法
				用函数指针保留。
				同时将最高的优先级的任务通过LED的方式改变，包括
...
//绿灯闪，灭函数，切换闪灭
#define DETECT_LED_G_TOGGLE() led_green_toggle()
#define DETECT_LED_G_ON() led_green_on()
#define DETECT_LED_G_OFF() led_green_off()
//流水灯闪灭函数
...
static void DetectDisplay(uint8_t num) {
  ...
  if (num == errorListLength + 1) {
    DETECT_LED_R_OFF();
    DETECT_LED_G_ON();
    last_num = errorListLength + 1;//无错误
  } else {
    //有错误
    DETECT_LED_G_OFF();
...
//		{10, 0, 12},  //camtopMotor
    ...
    {10, 0, 12},	//CHASSIS_CM1 CAN1
    {10, 0, 12},	//CHASSIS_CM2 CAN1
    {10, 0, 12},	//CHASSIS_CM3 CAN1
    {10, 0, 12},	//CHASSIS_CM4 CAN1
  };
